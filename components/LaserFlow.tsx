...
export const LaserFlow: React.FC<Props> = ({
  ...
  const startupTimeRef = useRef<number>(performance.now());
  ...
  useEffect(() => {
    startupTimeRef.current = performance.now(); // Reset startup time on mount
    ...
    const uniforms = {
      ...
      uFade: { value: (hasFadedRef.current || skipFade) ? 1 : 0 }
    };
    ...
    const io = new IntersectionObserver(
      entries => {
        // Be very lenient with intersection detection during page transitions
        // Only consider it out of view if intersection ratio is 0 (completely hidden)
        // Also, don't pause during initial load or page transitions
        const entry = entries[0];
        const wasInView = inViewRef.current;
        if (entry) {
          inViewRef.current = entry.intersectionRatio > 0;
        } else {
          inViewRef.current = true;
        }

        // If component just came back into view, ensure it's properly sized
        if (!wasInView && inViewRef.current) {
          setTimeout(() => setSizeNow(), 100); // Small delay to ensure DOM is ready
        }
      },
      { root: null, threshold: 0, rootMargin: '50px' } // Add root margin for better visibility detection
    );
    io.observe(mount);

    const onVis = () => {
      // Only pause when document is actually hidden and component is out of view
      // This prevents pausing during page transitions when loaders are showing
    ...
    const onCtxLost = (e: Event) => {
      ...
      console.warn('WebGL context lost for LaserFlow');
    };
    const onCtxRestored = () => {
      pausedRef.current = false;
      inViewRef.current = true; // Ensure we consider it in view after context restoration
      scheduleResize();
    };
    ...
    const animate = () => {
      ...
      // During page transitions, we want to keep rendering to avoid visual glitches
      // Only pause if the component is actually scrolled out of view or document is hidden
      // Give a 3-second grace period after mounting to handle page transitions
      const timeSinceStartup = performance.now() - startupTimeRef.current;
      const inGracePeriod = timeSinceStartup < 3000; // 3 seconds
      const shouldPause = !inGracePeriod && pausedRef.current && document.hidden && !inViewRef.current;
      ...
      if (!hasFadedRef.current && !skipFade) {
        // Use a much shorter fade duration for better navigation experience
        const fadeDur = 0.3;
      ...
      // Ensure renderer and canvas are still valid before rendering
